...
INTERNAL_MARKDOWN_CLEANUP_REASONING = (
    ...
    "\n2) 表格：对每个 GFM 表，先在脑中数清表头列数 N；确认存在分隔行且恰为 N 列；"
    "再自上而下逐行核对每行单元格数是否均为 N（含末行）；缺列或多列时优先按图像/上下文修形，勿硬凑。"
    "\n3) 易错格：最右列、靠边单元格、含勾选/符号的格、数字与单位、小数点与负号，逐格与附图核对。"
    "\n4) 竖线与转义：单元格内若需保留竖线，须为 \\|，禁止因竖线拆出「假列」。"
    "\n5) 收尾：脑中复查表格前后空行、行首无缩进，再输出纯 Markdown 正文。"
)
...
INTERNAL_VL_PAGE_REASONING = (
    ...
    "\n2) 表格：若存在表格，先定表头与列数 N，再写分隔行，再逐行填充；每写一行在脑中数一次是否 N 格；"
    "复杂表（多行表头、侧栏类别）先理清行列语义再落格，避免串行串列。"
    "\n3) 符号格：勾选、等级、限值、单位列易错，逐格核对，尤忌漏读最右列。"
    "\n4) 自检：输出前在脑中确认每张表均有分隔行、列数恒为 N、表格行首无缩进，再输出正文。"
)
...
def build_cleanup_messages(original_markdown: str) -> list[dict]:
    ...
    system = (
        ...
        "\n\n【表格（GFM pipe table）硬约束：必须严格遵守】"
        "\n- 任何表格都必须是 GitHub Flavored Markdown 的管道表格：每一行必须以 `|` 开头并以 `|` 结尾。"
        "\n- 必须包含分隔行，例如：`| --- | --- |`（每列至少 3 个 `-`，可含 `:` 对齐）。"
        "\n- 同一张表中：表头行、分隔行、每个数据行的列数必须完全一致；禁止缺列/多列。"
        "\n- 单元格内若出现竖线字符，必须写成 `\\|`，禁止让其被解析成额外列。"
        "\n- 禁止在表格行前加缩进（空格/Tab），否则很多编辑器会当成普通文本或代码块。"
